				   ...
				   struct btrfs_pending_snapshot *pending)
{
	...
	record_root_in_trans(trans, root, 0);
	btrfs_set_stack_root_last_snapshot(&root->root_item, trans->transid);
	...
	if (!(root_flags & BTRFS_ROOT_SUBVOL_RDONLY)) {
		...
		btrfs_set_stack_root_rtransid(new_root_item, 0);
	}
	btrfs_set_stack_timespec_sec(&new_root_item->otime, cur_time.tv_sec);
	btrfs_set_stack_timespec_nsec(&new_root_item->otime, cur_time.tv_nsec);
	btrfs_set_stack_root_otransid(new_root_item, trans->transid);
...
int btrfs_commit_transaction(struct btrfs_trans_handle *trans)
{
	...
	btrfs_set_stack_super_log_root_level(fs_info->super_copy, 0);
