			 ...
			 struct btrfs_ioctl_dedupe_args *dargs)
{
	...
	if (!test_bit(BTRFS_FS_DEDUPE_ENABLED, &fs_info->flags) ||
			!dedupe_info) {
		dargs->status = 0;
		dargs->blocksize = 0;
		dargs->backend = 0;
...
enable:
	...
	smp_wmb();
	set_bit(BTRFS_FS_DEDUPE_ENABLED, &fs_info->flags);
	return 0;
}
		     ...
		     struct btrfs_dedupe_hash *hash)
{
	...
	if (!test_bit(BTRFS_FS_DEDUPE_ENABLED, &fs_info->flags) || !hash)
		return 0;
...
int btrfs_dedupe_cleanup(struct btrfs_fs_info *fs_info)
{
	...
	clear_bit(BTRFS_FS_DEDUPE_ENABLED, &fs_info->flags);
	/* same as disable */
	smp_wmb();
			...
			struct btrfs_dedupe_hash *hash)
{
	...
	/*
	 * This function doesn't follow BTRFS_FS_DEDUPE_ENABLED as it will need
