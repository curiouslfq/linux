					       ...
					       struct btrfs_key *key)
{
	...
	root->node = read_tree_block(fs_info,
				     btrfs_stack_root_bytenr(&root->root_item),
				     generation);
	if (IS_ERR(root->node)) {
...
static void backup_super_roots(struct btrfs_fs_info *info)
{
	...
	btrfs_set_stack_backup_chunk_root_gen(root_backup,
			       btrfs_header_generation(info->chunk_root->node));
	btrfs_set_stack_backup_chunk_root_level(root_backup,
	...
	btrfs_set_stack_backup_extent_root_level(root_backup,
	...
	if (info->fs_root && info->fs_root->node) {
		...
		btrfs_set_stack_backup_fs_root_gen(root_backup,
			       btrfs_header_generation(info->fs_root->node));
		btrfs_set_stack_backup_fs_root_level(root_backup,
	...
	btrfs_set_stack_backup_dev_root_level(root_backup,
...
static int btrfs_check_super_valid(struct btrfs_fs_info *fs_info)
{
	 ...
	 * and one chunk
	 */
	if (btrfs_stack_super_sys_array_size(sb) > BTRFS_SYSTEM_CHUNK_ARRAY_SIZE) {
